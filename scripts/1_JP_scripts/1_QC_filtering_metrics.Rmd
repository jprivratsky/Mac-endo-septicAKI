...
knitr::opts_chunk$set(fig.width=12, fig.height=8,
                      ...
                      , cache=TRUE)
knitr::opts_knit$set(root.dir = '~/Desktop/Git-CLP0-24/')
```
...
############################################################################
```{r}
dir.create("./processedData/1_QC_filtering_metrics", recursive = T)
```
```{r}
...
v
pdf("./processedData/1_QC_filtering_metrics/1_VlnPlots_nFeature_RNA_nCount_RNA_percent.mito_per_sample_before_filtering.pdf"
    , width = 21
    , height = 9)
v
dev.off()
...
f
pdf("./processedData/1_QC_filtering_metrics/2_FeatureScatter_percent.mito_vs_nCount_RNA_per_sample_before_filtering.pdf")
f
dev.off()
```
...
f
pdf("./processedData/1_QC_filtering_metrics/3_FeatureScatter_nFeature_RNA_vs_nCount_RNA_per_sample_before_filtering.pdf")
f
dev.off()
```
...
pdf("./processedData/1_QC_filtering_metrics/4_Library_size.pdf"
    , width = 8
    , height = 9)
hist(experiment.aggregate$nCount_RNA, xlab = "Library sizes", breaks = 100, col = "grey80",
ylab = "Number of cells", main = "Distribution of library sizes
...
```{r}
saveRDS(experiment.aggregate, "./processedData/1_QC_filtering_metrics/5.experiment.aggregate.158.rds")
```
## QC with scater
```{r}
...
p
pdf("./processedData/1_QC_filtering_metrics/6_plotColData_detected_vs_sum.pdf"
    , width = 9
    , height = 8)
p
dev.off()
...
p
pdf("./processedData/1_QC_filtering_metrics/7_plotColData_subsets_Mito_percent_vs_sum.pdf"
    , width = 15
    , height = 9)
p
dev.off()
...
p
pdf("./processedData/1_QC_filtering_metrics/8_plotReducedDim_QC_metrics.pdf"
    , width = 9
    , height = 9)
p
dev.off()
...
legend.text = element_text(size = 7))
p
pdf("./processedData/1_QC_filtering_metrics/9_plotReducedDim_QC_metrics_manual_filtering.pdf"
    , width = 9
    , height = 9)
p
dev.off()
...
p <- plotColData(filtered, x = "sum", y="subsets_Mito_percent", 
    other_fields="sample.id") + facet_wrap(~sample.id)
p
pdf("./processedData/1_QC_filtering_metrics/10_plotColData_subsets_Mito_percent_vs_sum_post_filtering.pdf"
    , width = 15
    , height = 9)
p
dev.off()
...
pdf("./processedData/1_QC_filtering_metrics/11_plotHighestExprs.pdf"
    , width = 7
    , height = 7)
plotHighestExprs(experiment.aggregate.sce, exprs_values = "counts")
dev.off()
...
v
pdf("./processedData/1_QC_filtering_metrics/12_VlnPlots_nFeature_RNA_nCount_RNA_percent.mito_per_sample_post_filtering.pdf"
    , width = 21
    , height = 9)
v
dev.off()
...
f
pdf("./processedData/1_QC_filtering_metrics/13_FeatureScatter_percent.mito_vs_nCount_RNA_per_sample_post_filtering.pdf")
f
dev.off()
```
...
f
pdf("./processedData/1_QC_filtering_metrics/14_FeatureScatter_nFeature_RNA_vs_nCount_RNA_per_sample_post_filtering.pdf")
f
dev.off()
```

```{r}
saveRDS(filtered, "./processedData/1_QC_filtering_metrics/15.filtered.398.rds")
```
